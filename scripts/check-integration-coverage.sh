...
# requirement. Run from the module root: make check-integration
#
# Methods that are not endpoints (helpers / accessors) are listed, one name per
# line, in .integration-coverage-ignore.
set -euo pipefail
...
ignored=""
[ -f "$ignore_file" ] && ignored=$(grep -vE '^[[:space:]]*(#|$)' "$ignore_file" | sort -u)
